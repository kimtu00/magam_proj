...
export default function AuthTestPage() {
  ...
  const fetchOrCreateProfile = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "프로필 데이터 조회/생성 실패",
      );
      console.error("Fetch or create profile error:", err);
    } finally {
      setLoading(false);
    }
  }, [user, supabase]);
  ...
  const updateNickname = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "닉네임 업데이트 실패");
  ...
  }, [user, isLoaded, testConnection, fetchOrCreateProfile]);
    ...
    <div className="max-w-4xl mx-auto p-8">
        ...
        <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg flex items-start gap-2">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-red-600 mt-2">
              💡 <strong>해결 방법:</strong>
              <br />
              1. Supabase Dashboard에서 <code>profiles</code> 테이블이 생성되었는지
              확인
              <br />
      ...
      <div className="border rounded-lg">
        <div className="p-6 border-b">
          ...
          <p className="text-sm text-gray-600">
            Supabase의 profiles 테이블에 저장된 데이터입니다. RLS 정책에 따라
            ...
            <div className="space-y-4">
              <div className="p-4 bg-white border rounded-lg">
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <span className="font-semibold min-w-[120px]">역할:</span>
                    <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                      {profileData.role === "BUYER" ? "구매자" : "판매자"}
                    </span>
                  </div>
                  <div className="flex gap-2 items-center">
                    <span className="font-semibold min-w-[120px]">닉네임:</span>
                    {editingNickname ? (
                      ...
                      <>
                        <span>{profileData.nickname || "닉네임 없음"}</span>
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => {
      ...
      <div className="mt-8 p-6 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <ul className="text-sm text-blue-900 space-y-1 list-disc list-inside">
          ...
          <li>
            처음 로그인 시 Supabase profiles 테이블에 프로필 레코드가 자동으로
            생성됩니다
          </li>
